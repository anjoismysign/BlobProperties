...
import io.github.anjoismysign.bloblib.translatable.BlobTranslatableBlock;
...
public interface InternalProperty extends Property {
    ...
    default TranslatableBlock getLore(@NotNull Player player) {
        ...
        @Nullable TranslatableBlock translatableBlock = BlobLibTranslatableAPI.getInstance().getTranslatableBlock(reference, player.getLocale());
        if (translatableBlock == null) {
            BlobProperties.getInstance().getLogger().warning("Error fetching "+getMeta().typeName() + " '" + identifier()+"' lore ("+identifier() + "_lore TranslatableBlock)");
            translatableBlock = BlobTranslatableBlock.of(reference, "en_us", List.of("An error ocurred!", "Let an admin know."));
        }
